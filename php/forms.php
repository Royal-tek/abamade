...
if(isset($_POST['registrationBn'])){
  ...
  if($funct->viewUserby("email", $email)){
    $error = (empty($error)) ? "Email already exist" : "Username and Email Already Exist";
  }
  ...
  if(empty($error)){
    ...
    $a = array("user_id, f_name");
    $b = array("'$result'", "'$email'");
    $res = $controller->insert(CUSTOMERS, $a, $b);
    $_SESSION['msg'] = $funct->alert("Account Created Successfully", "success");
    //send email to user with info and link to download each file that was purchased 
    $message = '<html>
      <head>
        <title>Mail from Louis Chambers</title>
          <link href="https://fonts.googleapis.com/css?family=Open+Sans&display=swap" rel="stylesheet">
      ...
      <body>
        ...
        <p>
            Thank you for sigining up with us, your login detail are:<br />
            Username: '.$user.'<br />
            Password: '.$pwd.'
        </p>
        <p>Kindly Login to update your information.</p>
    ...
    $_SESSION['msg'] = $funct->alert($error, "warning");
  }
...
if(isset($_POST['loginBtn'])){
  ...
  if($user= $funct->viewUser($user, $pwd)){
    ...
    $error = "Username detail doesn't exit";
    $_SESSION['msg'] = $funct->alert($error, "warning");
    header('Location: ' . $_SERVER['HTTP_REFERER']);
  }
